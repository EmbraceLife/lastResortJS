...
(function() {
  ...
  */ 
  ...
  /** OFFICIAL
    ...
    * 
    */ 
  /** root: can be either self, global, this, or {}
   *  assign self -> global -> this -> {} to root
   */ 
  ...
  /* Save the previous value of the `_` into `previousUnderscore` */
  ...
  /** Ctor: an empty function to create other functions with different prototype
   * peek references to see its usages
   */ 
  var Ctor = function(){};

  /** _(obj) => object _ with property `_wrapped: obj`
   * @param obj
  ...
  */
   ...
   * functions. 
  */
  /** optimizeCb(func, context, argCount) => func with call as priority apply second
   * @param {*} func callback func
   * @param {*} context thisArg, usually an object
   * @param {*} argCount a number that count args for callback function
   * @returns a function
   */
  ...
  */
  ...
  /** OFFICAL
    ...
    */ 
  /** restArguments(func, startIndex): func(arg1, [args]) => func(arg1, arg2, ...) 
   // _.union(arrays) => restArgument =>  _.union([1,2], [3,4], ...) => rest: [[1,2],[3,4],...]
    // _.without(array, otherArrays) => restArgument => _.without([1,2,3,4], 2,4,...) => rest: [2,4,...]
    // var log = _.bind(console.log, console);
    // _.delay(func, wait, args) => restArgument => _.delay(log, 1000, 'logged later', 'and more', 'and less')
    */ 
  ...
  /** baseCreate(prototype): create a new empty object whose __proto__ is the arg `prototype` 
   * debugger;
   * var prototypeTest = {num: 1};
   * baseCreate(prototypeTest); => {}.__proto__ === {num:1}
   * baseCreate(3) => {}
   */ 
  ...
  */
  ...
  /** has(obj, path) ===> obj.hasOwnProperty(path)
  ...
  */
  ...
  /** OFFICIAL
    ...
    */ 
  /** isArrayLike(collection): check an arrayLike object
   * collection: must have a property 'length'
   * collection.length: must be a number >= 0 and smaller than 2**53-1
   * collection can be : array, object with property length
   * non arrayLike: any object without property length
   * isArrayLike([1,2,3]) => true
   * isArrayLike({length:1}) => true
   * checkArrayLike([1,2,3]) => true
   * checkArrayLike({length:1}) => true
   */ 
  ...
  /** tests
     // debugger;
    // root.eq(true, __.isArrayLike([1,2,3]));
    // root.eq(true, __.isArrayLike({1:"one", 2:"two", 3:"three", "length":3}));
    // root.eq(false, __.isArrayLike({1:"one", 2:"two", 3:"three"}));
    */ 

  var checkArrayLike = function(collection) {
    var returnFuncToGetValueFromKey = function(key) {
      return function(arrayOrObject) {
        return arrayOrObject == null ? void 0 : arrayOrObject[key];
      };
    };
    var getLengthFromObjectInGeneral = returnFuncToGetValueFromKey(key='length');
    var length = getLengthFromObjectInGeneral(arrayOrObject=collection);
    return typeof length == 'number' && length >= 0 && length <= MAX_ARRAY_INDEX;
  }
  /** tests
   // debugger;
    // root.eq(true, checkArrayLike([1,2,3]));
    // root.eq(true, checkArrayLike({1:"one", 2:"two", 3:"three", "length":3}));
    // root.eq(false, checkArrayLike({1:"one", 2:"two", 3:"three"}));
    */ 

  var checkArrayLikeSimplified = function(collection) {
    var length;
    if ([undefined, null].includes(collection)) {
      length = undefined; 
      /* as undefined.length, null.length cause TypeError, but NaN.length === undefined */
    } else {
  ...
  var argumentsAsArrayLike = function(){
    return arguments;
  }
  ...
  */
  /** _.keys(obj): return [] or [key1, key2, ...] 
    * obj: arrayLike or objects
    */ 
  _.keys = function(obj) {
